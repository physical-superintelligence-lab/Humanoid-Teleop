...
This repository contains USC GVL's custom Humanoid data collection pipeline, along with the re-engineering of Unitree's teleoperation of the H1 humanoid robots. For teleoperating and collecting data using the G1, please checkout the master branch.
...
## Install Unitree SDK2 Python and DDS Wrapper (Host only)
...
On H1's PC, install the image server's requirements
...
Copy `realsense_server.py` in the `teleop/image_server` directory to the PC of Unitree H1_2, and execute the following command **in the PC**:
...
1. Connect your host computer to H1. Then, set up your local IP address to 192.168.123.123 with netmask 255.255.255.0 on H1's network interface.
2. Connect both your computer and AVP to the same local router wifi in which you setup your cert with.
3. Open robot and set to develop mode (using remote control by pressing L1 + A, then L2 + R2).
4. On H1 PC: start the image server as above.
...
6. On host computer: run `python main.py --robot h1` under the `teleop/` directory and wait until the robot is in ready state. The terminal should signal both "master" and "worker" processes are waiting for starting signal.
7. On AVP, connect to robot using https://<host_ip_address_on_your_local_router>:8012/?ws=wss://<host_ip_address_on_your_local_router>:8012. Then, press `Enter VR` and then `Allow` to enter the web interface for teleoperating the H1.
