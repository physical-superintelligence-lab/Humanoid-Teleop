...
class HumanoidEnv:
    def __init__(self, policy_jit, robot_type="g1", device="cuda"):
        ...
        self.taskmaster = RobotTaskmaster(
            "debug",
            shared_data,
            robot_shm_array,
            teleop_shm_array,
    ...
    def extract_data(self):
        ...
        # print("self.ang_vel:", self.ang_vel)
    ...
    def run(self):        
        for i in range(int(self.sim_duration / self.sim_dt)):
            ...
            if i % self.sim_decimation == 0:
                ...
                # self.taskmaster.get_robot_data()
                # obs, self.extra_history_buf = self.taskmaster.get_ik_observation()
                # print("[DEBUG] observation data from real robot")
                # print("real_obs[:10]:", obs[:10])
                # print("real_obs.mean():", np.mean(obs))
                # print("real_obs.std():", np.std(obs))

                obs = self.get_observation()
                print("[DEBUG] observation data from mujoco")
                print("sim_obs[:10]:", obs[:10])
                print("sim_obs.mean():", np.mean(obs))
                print("sim_obs.std():", np.std(obs))
