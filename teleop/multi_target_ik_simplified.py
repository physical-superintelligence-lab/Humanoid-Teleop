...
class PinkIKSolver:
    ...
    def solve(self, q_init: np.ndarray, targets: dict,
        ...
        if self.q_prev is not None:
            self._arm_posture_task.set_reference(self.q_prev)
        else:
        ...
        if h0 is None or rpy0 is None:
            ...
            h_ref = float(h0)
            rpy_ref = np.array([rpy0[2], rpy0[1], rpy0[0]], dtype=np.float64)
    ...
    def assemble_q(self, name_to_pos, base_xyz=None, base_quat_wxyz=None):
        """
        Assemble a full URDF-format joint vector (q) from Mujoco joint positions.

        Args:
            name_to_pos: dict {joint_name: value or np.array(7,)} from Mujoco
