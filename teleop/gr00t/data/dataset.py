...
class CachedLeRobotSingleDataset(LeRobotSingleDataset):
    def __init__(self, img_resize: Optional[tuple[int, int]] = None, *args, **kwargs):
        """
...
class MixtureSpecElement(BaseModel):
    dataset_path: Union[list[Path], Path] = Field(..., description="The path to the dataset.")
...
class LeRobotMixtureDataset(Dataset):
    ...
    def compute_overall_statistics(
        per_task_stats: list[dict[str, dict[str, Union[list[float], np.ndarray]]]],
        dataset_sampling_weights: Union[list[float], np.ndarray],
